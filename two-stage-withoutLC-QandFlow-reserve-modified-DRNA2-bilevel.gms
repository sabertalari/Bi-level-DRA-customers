$CALL GDXXRW dataexcelWind_new3_twostage_newDR.xlsx Index=ParamsAndSets!A1
$GDXIN dataexcelWind_new3_twostage_newDR.gdx
...
N_DR(N)
;
$load t,PVscen,Windscen,w,N_PV,S, R SL conex conex_sin N_DG N_REN N_REN2 PARL PDQ Prob SWITCH N_TN CONDR N_DR
alias(N,NP,i,j)  ;
alias(t,tAR,tDEP);
*alias(t,k);
...
IMIN2                              Valor de corriente mínimo en la red          [A]
...
$Load Demand_P,Windexp, PVexp,  LineData,  demand,   P_DG_MAX, P_DG_MIN, PV_S, Wind_S  Price_Energy,Price_Reserve,P_del_Res,Price_Reg_Up,Price_Reg_Down,P_del_Reg_Down,P_del_Reg_Up,Cap_payment_price,Cap_Reg_Down,Cap_Reg_Up  ,Landa_DR
...
*+Res_up(N,t,w)
P_DG_limit(N,t,w)$ N_DG(N)..                           P_DG(N,t,w) =L= .3;

reserve_limit_ups(N,t,w,s)$(ord(N)=1)..                           Res_ups(N,t,w,s)=l=Res_up(N,t,w);
reserve_limit_dns(N,t,w,s)$(ord(N)=1)..                            Res_up(N,t,w)+PSS(N,t,w)=l=1;
reserve_limit_dns_L(N,t,w,s)$(ord(N)=1)..                            Res_up(N,t,w)+PSS(N,t,w)=g=-1;
DR_limit(N,NP,t)..     P_DR(N,NP,t)=l= .2*Pdem(N,'pt',t);
DR_limit2(N,t)..     P_DR1(N,t)=l= .2*Pdem(N,'pt',t);

*---------------------
*BiLEVEL-DUAL------------------------------------------------------------------------------------------
dual1..                     sum((t,n),(P_DR1(N,t)*Landa_DR(t,N)$N_DR(N)+sum(NP$ CONDR(N,NP),P_DR(N,NP,t)*Landa_DR(t,NP))))=e=sum((t,n),(.2*Pdem(N,'pt',t)*Y_Dual1(t,N)$N_DR(N)+.2*Pdem(N,'pt',t)* Y_Dual2(t,N)$N_DR(N)+sum(NP$ CONDR(N,NP), .35*Pdem(N,'pt',t)*Y_Dual3(t,N,NP))));
dual2(t,n)..                     P_DR1(N,t)$N_DR(N)+sum(NP$ CONDR(N,NP),P_DR(N,NP,t))=l=.35*Pdem(N,'pt',t);
dual3(t,n)..                     Y_Dual1(t,N)$N_DR(N)+Y_Dual2(t,N)$N_DR(N)=g=Landa_DR(t,N);
dual4(t,n)..                     Y_Dual1(t,N)$N_DR(N)+sum(NP$ CONDR(NP,N),(Y_Dual3(t,N,NP)-Landa_DR(t,NP)))=g=0;
DR_result(t)..     DR_final(t)=e= sum(N,Pdem(N,'pt',t)- P_DR1(N,t)-sum(NP$ CONDR(N,NP),P_DR(N,NP,t)));
LOAD_TOTAL(t)..                  load_final(t)=e= sum(N,Pdem(N,'pt',t));
*active_load(t,n)..               Each_node_load(t,n)=e=Pdem(N,'5',t);
*reserve_DR(N,t,s)..                                        TEst(N,t,s)=e= sum(w,PSS_S(N,t,w,s)$(ord(N)=1)-PSS(N,t,w)$(ord(N)=1));
...
*testsecondstage(N,t,w,s)= NumEolica *(P_RNs(N,t,w,s)$ N_REN(N)-P_RN(N,t,w)$ N_REN(N))+ NumEolica *(P_RNs2(N,t,w,s)$ N_REN2(N)-P_RN2(N,t,w)$ N_REN2(N));
...
*Tolerancia para la convergencia de la optimización con variables binarias
OPTION OPTCR =0.15;
* Número máximo de iteraciones que se le permite haceral optimizador
OPTION ITERLIM =1e8;
* Número máximo de iteraciones que se le permite hacer al optimizador
OPTION RESLIM =1e10;
*OPTION threads=0;
OPTION DECIMALS =8;
OPTION mip=cplex;
SOLVE  totalcost USING MIP MINIMIZING ProfitTest;
*Display E_BAL_POT_P.l;
*display   testsecondstage;
...
**-------------------------------------------------------------------------------------------------------------**
*$LIBINCLUDE XLDUMP  DR_final.l DRresult_secondwork3.xls Sheet1
*$LIBINCLUDE XLDUMP  load_final.l DRresult_secondwork3.xls Sheet2
*$LIBINCLUDE XLDUMP  P_DR1.l DRresult_secondwork3.xls Sheet3
*$LIBINCLUDE XLDUMP  P_DR.l DRresult_secondwork3.xls Sheet4
